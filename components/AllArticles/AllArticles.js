...
const AllArticles = ({ blok }) => {
  ...
  const filteredArticles = articles.filter(article => {
    return article.tag_list.some(tag => checkboxes.some(checkBox => checkBox.active && checkBox.tagName === tag));
  });
  ...
  return (
    ...
    const activeTags = checkboxes.filter(tag => tag.active).map(tag => tag.tagName.toLowerCase());

    if (activeTags.length === 0 || activeTags.every(tag => articleTags.includes(tag))) {
      return <ArticleTeaser article={article.content} tags={article.tag_list} key={article.uuid} onclick={applyFilter} />;
    }
